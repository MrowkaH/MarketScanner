"""
PPI Predictor - Data Collector

Fetches historical PPI data and related economic indicators
...
class DataCollector:
    """Collects PPI and related economic data from FRED API."""

    def __init__(self):
        if not FRED_API_KEY:
            raise ValueError(
                "FRED_API_KEY not found. Set it in PPI_Predictor/.env"
            )
    ...
    def fetch_ppi_target(self) -> pd.Series:
        """Fetch the primary PPI series (target variable)."""
        print(f"[DataCollector] Fetching target series: {PPI_TARGET_SERIES}")
        series = self._fetch_series(PPI_TARGET_SERIES)
        series.name = "PPI"
        return series
    ...
    def fetch_all(self) -> pd.DataFrame:
        """
        Fetch target PPI + all feature series, merge into a single
        monthly DataFrame aligned by date index.
        """
        target = self.fetch_ppi_target()
