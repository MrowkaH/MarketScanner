"""
PPI Predictor - ML Prediction Engine
...
Dual approach:
  ...
  2. CLASSIFICATION – Gradient Boosting + Random Forest classifier to produce
     probability distribution over discrete PPI MoM bins (e.g. -0.5%, -0.4%, …, +0.5%)
...
class Predictor:
    """ML engine for PPI prediction – regression + classification."""
    ...
    def predict(self, df: pd.DataFrame) -> dict:
        ...
        for proba in all_probas.values():
            # Align to full bin set
            aligned = np.zeros(len(clf_classes))
            clf_obj = list(self.clf_models.values())[0]
    ...
    def _bin_target(self, y_train: np.ndarray, y_test: np.ndarray):
        ...
        def digitize(y):
            ...
            for val in y:
                if val < edges[0]:
                    # Either overflow-low or first center bin
                    if val < low - step / 2:
                        result.append(labels[0])  # <-1.0%
                    else:
                        result.append(labels[1])  # first center
                elif val >= edges[-1]:
                    # Either last center or overflow-high
                    if val >= high + step / 2:
    ...
    def _build_reg_model(self, name: str, params: dict):
